...
void main() {
	float Dis = uZDistance *100;
	v_world_space = Dis * points[indices[v_instanceID[0]*3]];
	gl_Position = uProjectionMatrix * vec4(mat3(uModelViewMatrix) * v_world_space, 1);
	EmitVertex();

	v_world_space = Dis * points[indices[v_instanceID[0]*3+1]];
	gl_Position = uProjectionMatrix * vec4(mat3(uModelViewMatrix) * v_world_space, 1);
	EmitVertex();

	v_world_space = Dis * points[indices[v_instanceID[0]*3+2]];
	gl_Position = uProjectionMatrix * vec4(mat3(uModelViewMatrix) * v_world_space, 1);
	EmitVertex();
	EndPrimitive();
}
...
vec2 latlong(vec3 v) {
    ...
    vec2 texCoord = vec2(1.0 - (phi + PI) / (2.0 * PI), theta / PI);
    return texCoord;
}
