...
uniform vec3 u_cameraPosition; // TOO CHECK
uniform mat4 u_rotationMatrix; // TOO CHECK
...
void main() {
	...
	if (u_directOutputPass) {
		...
		fb_color = texture(u_screenTexture, fragUV);
		...
		if (u_accumulatedPasses > 0) {
			...
			if (raycast(Ray(cameraRay.origin, offsetDirection), hitPoint)) {
				fb_color += vec4(hitPoint.material.emission*hitPoint.material.emissionStrength*u_bloomIntensity, 1.0);
			}
